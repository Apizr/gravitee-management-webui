  ...
  getNotifiers(scope: HookScope, id: string): IHttpPromise<any> {
    switch (scope) {
      ...
      case HookScope.PORTAL:
        return this.$http.get(this.portalgCfgURL + id + "/notifiers");
      default:
        break;
    }
  }

  getGenericNotifications(notifierId: string, scope: HookScope, id: string): IHttpPromise<any> {
    switch (scope) {
      case HookScope.APPLICATION:
        return this.$http.get(this.applicationsURL + id  + '/genericnotifications?notifier=' + notifierId);
      case HookScope.API:
        return this.$http.get(this.apisURL + id + '/genericnotifications?notifier=' + notifierId);
      case HookScope.PORTAL:
        return this.$http.get(this.portalgCfgURL + '/genericnotifications?notifier=' + notifierId);
      default:
        break;
    }
  }

  saveConfig(cfg: GenericNotificationConfig): IHttpPromise<any> {
    if (cfg.referenceType === "API") {
      return this.$http.post(this.apisURL + cfg.referenceId + '/genericnotifications', cfg);
    } else if (cfg.referenceType === "APPLICATION") {
      return this.$http.post(this.applicationsURL + cfg.referenceId + "/genericnotifications", cfg);
    } else if (cfg.referenceType === "PORTAL") {
      return this.$http.post(this.portalgCfgURL + '/genericnotifications', cfg);
    } else {
      return;
    }
  }
